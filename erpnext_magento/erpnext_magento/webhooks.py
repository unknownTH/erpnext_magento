...
def magento_webhook(f):
	"""
	A decorator thats checks and validates a Magento Webhook request.
	"""
	...
	def wrapper(*args, **kwargs):
		...
		try:
			webhook_topic = frappe.local.request.headers.get('X-Magento-Topic')
			webhook_hmac	= frappe.local.request.headers.get('X-Magento-Hmac-Sha256')
			webhook_data	= frappe._dict(json.loads(frappe.local.request.get_data()))
		except:
			raise ValidationError()

		# Verify the HMAC.
		if not _hmac_is_valid(frappe.local.request.get_data(), get_magento_settings().password, webhook_hmac):
