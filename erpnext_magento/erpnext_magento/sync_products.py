...
def sync_products(price_list, warehouse):
	...
	# sync_erpnext_items(price_list, warehouse, magento_item_list)
...
def make_item(warehouse, magento_item, magento_item_list):
	...
	if has_variants(magento_item):
		...
		create_item_variants(magento_item, warehouse, attributes, magento_variants_attr_list, magento_item_list=magento_item_list)

	else:
		magento_item["variant_id"] = magento_item['variants'][0]["id"]
		create_item(magento_item, warehouse, magento_item_list=magento_item_list)

def add_item_weight(magento_item):
	magento_item["weight"] = magento_item['variants'][0]["weight"]
	magento_item["weight_unit"] = magento_item['variants'][0]["weight_unit"]

def has_variants(magento_item):
	if len(magento_item.get("options")) >= 1 and "Default Title" not in magento_item.get("options")[0]["values"]:
		return True
	return False
...
def create_attribute(magento_item):
	attribute = []
	# magento item dict
	for attr in magento_item.get('options'):
...
def create_item(magento_item, warehouse, has_variant=0, attributes=None,variant_of=None, magento_item_list=[]):
	item_dict = {
		...
		"sync_with_magento": 1,
		"is_stock_item": 0,
		"item_code": magento_item.get("title"),
		"item_name": magento_item.get("title"),
		"description": magento_item.get("body_html") or magento_item.get("title"),
		"magento_description": magento_item.get("body_html") or magento_item.get("title"),
		"item_group": get_item_group(magento_item.get("product_type")),
		"has_variants": has_variant,
		"attributes":attributes or [],
		"stock_uom": magento_item.get("uom") or _("Nos"),
		"stock_keeping_unit": magento_item.get("sku") or get_sku(magento_item),
		"default_warehouse": warehouse,
		"image": get_item_image(magento_item),
		"weight_uom": magento_item.get("weight_unit"),
		"net_weight": magento_item.get("weight"),
		"default_supplier": get_supplier(magento_item),
		"default_material_request_type": "Manufacture"
	}
	item_dict["web_long_description"] = item_dict["magento_description"]
...
def sync_erpnext_items(price_list, warehouse, magento_item_list):
...
def sync_item_with_magento(item, price_list, warehouse):
	...
	item_data = { "product":
		{
			...
			"body_html": item.get("magento_description") or item.get("web_long_description") or item.get("description"),
	...
	if item.get("has_variants") or item.get("variant_of"):
		...
		item_data["product"]["body_html"] = item.get("magento_description") or item.get("web_long_description") or item.get("description")
